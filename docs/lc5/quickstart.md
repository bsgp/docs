...
드디어 화면에 무언가 나타났습니다. 하지만 여전히 너무 허전합니다. 무엇보다 라벨이 없으니 어떤 Input인지 알 수가 없군요.
...
본 예제에서는 간단한 코드 샘플을 위해 State를 활용하여 폼 전송을 구현했지만, 실제 LC5에서는 S3와 연결하여 DB에 데이터를 넣을 수 있는 모듈이 내장되어 있습니다.
